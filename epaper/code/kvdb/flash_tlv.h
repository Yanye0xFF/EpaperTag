...
typedef struct _tlv_sector {
    // public:
    uint32_t major_sector; // sector address, aligned with 'sector_size'
    uint32_t minor_sector; // sector address, aligned with 'sector_size'
    uint16_t sector_size;  // physical sector size, unit:byte
    // private:
    uint16_t dirty_blocks; // 脏块数量, 包括写入后校验失败块，标记删除块，Meta域异常坏块
    uint32_t work_sector; // 当前工作扇区地址，major_sector或minor_sector其中一个
    uint32_t mark_address; // 写入重复Tag时，旧Tag的地址(新Tag写入完成后标记旧Tag删除)
} tlv_sector_t;
